...
func (fp *FileParser) Parse(src []byte) (*File, error) {
	...
	for _, v := range pf.Decls {
		...
		if dec, ok := v.(*ast.GenDecl); ok {
			...
			default:
				fmt.Println("Skipping unknown Token Type")
			}
		}
	}
...
func (fp *FileParser) parseType(ds []ast.Spec, f *File) {
	for _, sp := range ds {
		...
		if !ok {
			fmt.Println("Type spec is not TypeSpec type, odd, skipping")
			continue
		}
...
func (fp *FileParser) parseImports(ds []ast.Spec) []NamedTypeValue {
	...
	for _, sp := range ds {
		...
		if !ok {
			fmt.Println("Import spec is not ImportSpec type, odd, skipping")
			continue
		}
		ip := NewNameType("", "")
		if isp.Name != nil {
...
func (fp *FileParser) parseVars(ds []ast.Spec) []NamedTypeValue {
	...
	for _, sp := range ds {
		...
		if !ok {
			fmt.Println("Var spec is not ValueSpec type, odd, skipping")
			continue
		}
...
func (fp *FileParser) parseConstants(ds []ast.Spec) []NamedTypeValue {
	...
	for _, sp := range ds {
		vsp, ok := sp.(*ast.ValueSpec)
		if !ok {
			fmt.Println("Constant spec is not ValueSpec type, odd, skipping")
			continue
		}
		fst := token.NewFileSet()
		bt := bytes.NewBufferString("")
...
func (fp *FileParser) getTypeFromExp(e ast.Expr) string {
	...
	case *ast.SelectorExpr:
		// fmt.Println("Type Selector, i.e. a third-party type")
		selectorIdent := fp.getTypeFromExp(k.X)
		tp = fmt.Sprintf("%s.%s", selectorIdent, k.Sel.Name)
	case *ast.StarExpr:
